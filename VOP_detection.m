...
% Code to extract the Vowel regions
start_idx = floor(x(pos_peaks_idx(1))*fs);
end_idx = floor(x(pos_peaks_idx(length(pos_peaks_idx)))*fs);

% disp(start_idx);  // starting frame
% disp(end_idx);    // ending frame

j = 1;
vowel_region = zeros(end_idx-start_idx+1);
for i=start_idx:end_idx
    vowel_region(j) = data(i);
    j = j + 1;
end
...
filename = strcat(audio_file, '_vowel', '.wav');
audiowrite(filename, vowel_region, fs);
